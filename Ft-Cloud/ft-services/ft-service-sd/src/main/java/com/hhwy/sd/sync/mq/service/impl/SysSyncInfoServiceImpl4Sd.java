    ...
    public void pushKcsjDesignDocumentApproval(List<KcsjDesignDocumentApproval> list) {
        try{
            ...
            if(!CollectionUtils.isEmpty(list)){
                ...
                KcsjDesignDocumentApproval approval=new KcsjDesignDocumentApproval();
                if(prjInfo.get("projectId") != null)approval.setProjectId(Long.parseLong(prjInfo.get("projectId").toString()));
                JSONObject json = JSONObject.parseObject(JSONObject.toJSONString(approval));
                finalList.add(json);
                rocketMQTemplate.convertAndSend("kcsj_design_document_approval:tenantSuccess", JSONObject.toJSONString(finalList));
