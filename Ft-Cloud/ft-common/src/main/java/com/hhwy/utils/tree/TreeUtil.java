...
public class TreeUtil {
    ...
    /**
     * 导出树形集合，按照树形结构重新排序 继承treeNode实体类用该方法
     *
     * @param list
     * @return
     */
    ...
    /**
     * 导出树形集合，按照树形结构重新排序 继承treeNodeBase实体类用该方法
     *
     * @param list
     * @return
     */
    ...
    private static <T extends TreeNodeBase> void chooseChild(List<T> list, Class<R> t1, Map<R, List<T>> childMap,
        List<T> resuList, Set<R> idSet) {
        if (CollectionUtils.isEmpty(list)) {
            return;
        }
        //排序
        list.sort((v1, v2) -> {
            return CompareUtil.compare(v1.getSort(), v2.getSort());
        });
        for (int i = 0; i < list.size(); i++) {
            T t = list.get(i);
     ...
     * @return
     */
    public static <T extends TreeNode<T>> List<T> treeToListWithoutId(List<T> source) {
        List<T> result = new ArrayList<>();
        if (CollectionUtils.isEmpty(source)) {
            return result;
        }
        ...
        for (T node : source) {
            node.setSort(sort++);
            splitWithoutId(node, result);
        }
        return result;
    }

    private static <T extends TreeNode<T>> void splitWithoutId(T node, List<T> resultList) {
        int sort = 1;
        List<T> children = node.getChildren();
        node.setChildren(null);
        resultList.add(node);
